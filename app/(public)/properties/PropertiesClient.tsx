...
import { Search, MapPin, Bed, Bath, Square, Heart, Eye, X, SlidersHorizontal } from 'lucide-react';
...
const FALLBACK = 'https://images.unsplash.com/photo-1486325212027-8081e485255e?w=500&q=80';
...
export default function PropertiesClient({ initialProperties, initialTotal }: Props) {
  ...
  const sectors = filters.province && filters.district
    ? getSectors(filters.province as Province, filters.district)
    : [];
  ...
  // Only fetch when user actively changes filters (not on initial mount — server already provided data)
  useEffect(() => {
    if (!isFiltered) return;
    fetchProperties(filters, page);
  }, [filters, page, isFiltered, fetchProperties]);
          ...
          <div className="flex gap-3 mt-5 max-w-2xl">
            ...
            <Button variant="outline" className="border-white/20 text-white hover:bg-white/10 gap-2"
              ...
              Filters {activeFilterCount > 0 && (
                <span className="w-5 h-5 bg-primary rounded-full text-xs flex items-center justify-center">{activeFilterCount}</span>
              )}
            </Button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <select value={filters.category} onChange={e => updateFilter('category', e.target.value)}
                  className="w-full h-9 rounded-md border border-input bg-background px-2 text-sm focus:outline-none">
              ...
              <div>
                ...
                <select value={filters.province}
                  onChange={e => { updateFilter('province', e.target.value); updateFilter('district', ''); updateFilter('sector', ''); }}
                  className="w-full h-9 rounded-md border border-input bg-background px-2 text-sm focus:outline-none">
                  <option value="">Any</option>
              ...
              <div>
                ...
                <select value={filters.district} onChange={e => updateFilter('district', e.target.value)}
                  ...
                  className="w-full h-9 rounded-md border border-input bg-background px-2 text-sm focus:outline-none disabled:opacity-50">
                  <option value="">Any</option>
        ...
        {total > 12 && (
          <div className="flex justify-center gap-2 mt-8">
            <Button variant="outline" disabled={page === 1}
              onClick={() => { setPage(p => p - 1); setIsFiltered(true); }}>Previous</Button>
            <span className="flex items-center px-4 text-sm text-muted-foreground">Page {page}</span>
            <Button variant="outline" disabled={properties.length < 12}
              onClick={() => { setPage(p => p + 1); setIsFiltered(true); }}>Next</Button>
          </div>
        )}
      </div>
    </div>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
      <Card className="overflow-hidden group border-0 shadow-md hover:shadow-xl transition-shadow">
        <div className="relative h-48">
          <Image src={property.images[0] || FALLBACK} alt={property.title}
            fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
          <div className="absolute top-3 left-3 flex gap-1.5">
            <Badge variant={property.category === 'sale' ? 'success' : 'info'} className="text-xs capitalize">
        ...
        <CardContent className="p-4">
          ...
          {property.plotSize && !property.bedrooms && (
