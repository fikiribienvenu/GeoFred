...
import type { AgentData, AgentStats } from '@/lib/data';
...
export default function AgentNetworkSection({ initialAgents, initialStats }: Props) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }}>
            ...
            <p className="text-muted-foreground leading-relaxed mb-6">
              GeoFredE-Terra State has a network of certified agents covering all major districts and sectors across Rwanda.
              Every agent is verified, trained, and assigned to serve specific geographic areas.
            </p>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              {[
                { icon: Users, value: initialStats.total > 0 ? `${initialStats.total}+` : '0', label: 'Active Agents' },
                { icon: MapPin, value: initialStats.districts > 0 ? `${initialStats.districts}` : '0', label: 'Districts' },
                { icon: Star, value: initialStats.rating, label: 'Avg. Rating' },
